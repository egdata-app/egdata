...
const eat = (a: DecodeState) => {
  ...
  for (j = a._idx; j < a._length && ((c = a._input.charAt(j)), c !== "~" && c !== ")"); j++) {}
...
const encode = (value: any, out: string[], rich: boolean | undefined, depth: number) => {
  let t,
    T = typeof value;
...
const clean = (str: string) => {
  ...
  while (i < str.length) {
    ...
    if (c === 37) {
      ...
      i = j = 0;
    } else if (c === 32 || c === 10 || c === 13 || c === 0 || c === 8232 || c === 8233) {
