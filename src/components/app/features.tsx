...
export function GameFeatures({ id, size: _size = "md" }: { id: string; size?: FeaturesSizes }) {
  ...
  return (
    <div className="w-full justify-center items-center flex absolute bottom-3 right-0 left-0 px-2">
      <div className="h-auto flex flex-wrap gap-3 w-fit max-w-full bg-surface-active py-2 px-6 rounded-lg justify-center items-center text-text-primary backdrop-blur-sm">
        {launcher && (
          <TooltipProvider>
  ...
  return (
    <div className="flex flex-row gap-2 w-full justify-start items-center">
      {launcher && (
        <TooltipProvider>
          <Tooltip delayDuration={50}>
            <TooltipTrigger className="bg-surface-hover rounded-full p-2 text-center">
              {launcher.icon}
            </TooltipTrigger>
            <TooltipContent>
              <div className="text-center">{launcher.name}</div>
