...
export function SectionsNav({
  ...
  return (
    <ControlledScrollArea orientation={isHorizontal ? "horizontal" : "vertical"}>
      <nav className={cn("w-full max-w-3xl", isHorizontal ? "flex-row" : "flex-col w-48")}>
        <ul
          className={cn(
            isHorizontal
              ? "inline-flex h-9 items-center justify-center space-x-1 rounded-md bg-muted/50 p-1 text-muted-foreground"
              : "flex flex-col space-y-1 rounded-md bg-muted/50 p-1 text-muted-foreground",
          )}
        >
          {links.map((link) => (
            <li
